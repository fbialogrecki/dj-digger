...
# Tempo, when anyone bothered to say


def records_with_tempo(count, tempos):
    records = synthetic_records(count)
    for record, tempo in zip(records, tempos):
        if tempo:
            record.track.tags = [f"{tempo}bpm"]
    return records
...
def test_a_crate_that_never_mentions_tempo_has_no_bpm_column(state):
    """A column of dashes would cost the title three columns to say nothing."""
    ...
    async def scenario():
        async with app.run_test() as pilot:
            await pilot.pause()
            assert "BPM" not in column_labels(app)

    run(scenario)


def test_a_crate_that_does_gets_the_column(state):
    app = make_app(records_with_tempo(3, [150, None, 162]), state)

    async def scenario():
        async with app.run_test() as pilot:
            await pilot.pause()
            assert "BPM" in column_labels(app)
            table = app.query_one("#tracks", DataTable)
            assert [str(table.get_row_at(index)[-1]) for index in range(3)] == [
                "150",
                "-",
                "162",
            ]
...
def test_the_column_goes_away_with_the_crate_that_needed_it(state):
    app = make_app(records_with_tempo(2, [174, None]), state)
    ...
    async def scenario():
        async with app.run_test() as pilot:
            ...
            app.load_records(synthetic_records(2))
            await pilot.pause()
            assert "BPM" not in column_labels(app)
...
def test_the_title_still_takes_what_is_left_beside_the_tempo(state):
    app = make_app(records_with_tempo(3, [150, 150, 150]), state)
